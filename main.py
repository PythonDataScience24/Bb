...
def sort_by(
    dataframe, type, mode
):  # returns a sorted dataframe and expects a column type and the way to sort
...
def filter_for(
    dataframe, type, argument
):  # returns a filtered dataframe and expects a column type and the argument to search for
    columns = get_columns(dataframe)
    if not type in columns:
        return create_dataframe(columns)
